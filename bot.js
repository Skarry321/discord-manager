...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    let category = interaction.guild.channels.cache.find(c => c.type === 4 && (c.name.toLowerCase().includes('ticket') || c.name.toLowerCase().includes('support')));
    ...
    if (staffRole) {
      perms.push({ id: staffRole.id, allow: ['ViewChannel', 'SendMessages', 'ReadMessageHistory', 'ManageChannels'] });
    }
    ...
    const closeBtn = new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId('close_' + channel.id).setLabel('\uD83D\uDD12 Close ticket').setStyle(ButtonStyle.Danger)
    );
